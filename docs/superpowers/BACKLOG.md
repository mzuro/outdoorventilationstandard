...
- **Hood-geometry diagram + tool page depict the 1.70 (open-air/wind-exposed) hood, not the 1.38 base recommendation.** `static/diagrams/hood-geometry-comparison.svg` and `content/tools/hood-geometry-comparison.md` render Gas-Med-30" with a 70" hood (70/41=1.70), 17" overhang, ~95% capture. W6 relabeled them "open-air (K=1.70)" (label-only, no rendered number changed) because a false 1.38 relabel would contradict the drawn 70" geometry. Optional: if Mark wants these to instead illustrate RB-002's *base* recommendation, regenerate at 57" hood / ~10.5" overhang with recomputed capture % (a geometry + capture-number change, out of prose-only scope).
