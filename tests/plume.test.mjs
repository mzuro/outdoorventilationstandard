...
test('radius grows linearly from source radius', () => {
  assert.equal(plumeRadius(0), 14);
  assert.ok(Math.abs(plumeRadius(100) - (14 + 0.11 * 100)) < 1e-9);
});
test('velocity constant in near field then decays as cbrt', () => {
  assert.equal(centerlineVelocity(6), 400);
  assert.equal(centerlineVelocity(12), 400);
  assert.ok(Math.abs(centerlineVelocity(96) - 400 * Math.cbrt(12 / 96)) < 1e-9);
});
